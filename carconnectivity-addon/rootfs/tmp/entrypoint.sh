...
LEGACY_FILE="/config/carconnectivity.UI.json"
...
# Config precedence: expert.json > custom page (carconnectivity.json) >
# legacy pre-configui config (carconnectivity.UI.json) > defaults
SRC_CONFIG=""
...
elif [ -f "${LEGACY_FILE}" ] && validate_json "${LEGACY_FILE}" >/dev/null 2>&1; then
    # Config generated by a pre-configui version of the addon: keep it running
    # after the upgrade. The page imports (and renames to .bak) this file the
    # first time the configuration is opened and saved.
    SRC_CONFIG=${LEGACY_FILE}
    color_echo "${GREEN}" "♻️ Previous add-on configuration applied (${LEGACY_FILE}). Open the configuration page and save to adopt it."
else
    if [ "${EXPERT_EXISTS}" = "true" ] && [ "${EXPERT_SYNTAX}" != "true" ]; then
